...
int LaunchToolRuntime() {
    ...
    *lastSlash = L'\0';
    ...
    wchar_t rootPath[kPathBufferSize] = {};
    std::wmemcpy(rootPath, launcherPath, wcslen(launcherPath) + 1);

    wchar_t* toolsSlash = wcsrchr(rootPath, L'\\');
    if (toolsSlash == nullptr) {
        return 1;
    }
    *toolsSlash = L'\0';
    ...
    if (_snwprintf_s(
            ...
            kPathBufferSize,
            _TRUNCATE,
            L"%s\\bin\\VoxHeaderToolRuntime.exe",
            rootPath) < 0) {
        return 1;
    }
