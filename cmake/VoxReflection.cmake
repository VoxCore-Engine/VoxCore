...
function(voxcore_append_generated_headers OUTPUT_VARIABLE ROOT_DIR GENERATED_ROOT)
    ...
    foreach(header IN LISTS ARGN)
        if(header MATCHES "/Public/")
            file(RELATIVE_PATH relative_path "${ROOT_DIR}/Public" "${header}")
            ...
            if(relative_directory STREQUAL "")
                ...
                list(APPEND generated_headers "${GENERATED_ROOT}/Public/${relative_directory}/${base_name}.generated.h")
            endif()
        elseif(header MATCHES "/Private/")
            file(RELATIVE_PATH relative_path "${ROOT_DIR}/Private" "${header}")
            get_filename_component(relative_directory "${relative_path}" DIRECTORY)
            get_filename_component(base_name "${relative_path}" NAME_WE)
            if(relative_directory STREQUAL "")
                list(APPEND generated_headers "${GENERATED_ROOT}/Private/${base_name}.generated.h")
            else()
                list(APPEND generated_headers "${GENERATED_ROOT}/Private/${relative_directory}/${base_name}.generated.h")
            endif()
        endif()
    endforeach()
